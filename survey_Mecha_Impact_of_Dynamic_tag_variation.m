...
preambleForEstChannel_addTags = survey_Mecha_funcGeneratePreamble(seqLenForEstChannel,numTags+numAddTags);

maxNumPackets = 10000; % The maximum number of packets at an SNR point

S = numel(snr); 
numBitErrs = zeros(S,numTags+numAddTags);
berEst = zeros(S,numTags+numAddTags);

numDataSubcarrier = 52;
numPilotSubcarrier = 4;
estH = ones(numDataSubcarrier+numPilotSubcarrier,numTags+numAddTags);
...
for i = 1:S
    ...
    while  n<=maxNumPackets
        disp(['snr: ',num2str(snr),' dB -> ','n: ',num2str(n),'-th packet']);
        %%%%%*** TX side ***%%%%%
        ...
        for chan_tx_tag_idx1 = 1:(numTags+numAddTags)
        ...
        numPayload1 = numTagData-seqLenForEstChannel*(numTags+numAddTags);
        for tag_idx2 = numTags+1:(numTags+numAddTags)
            payload1 = randi([0,1],numPayload1,1);
            actualPayloadBits_addTags(:,tag_idx2-numTags+1) = payload1;
            tagData(:,tag_idx2) = [preambleForEstChannel_addTags(:,tag_idx2);payload1];
        end
        
        % backscatter operation
        for tag_idx3 = 1:(numTags+numAddTags)
            bxSig{tag_idx3} = survey_Mecha_funcBackscatter(exSig(:,tag_idx3),tagData(:,tag_idx3),1);
        end
        
        %%%%%***** Backscatter channel & AWGN ***%%%%%
        for chan_tag_rx_idx1 = 1:(numTags+numAddTags)
            reset(tgnChannel);
        ...
        for rx_idx1 = 1:(numTags+numAddTags)
            rx = rx + bxSig{rx_idx1};
        end
        ...
        % LS estimator
        for rx_idx2 = 1:(numTags+numAddTags)
        ...
        payload_ofdmDemod = ofdmDemod(:,1+seqLenForEstChannel*(numTags+numAddTags)+1:end-numSymForTailPad); 
        payload_ofdmDerived = ofdmDerived(:,1+seqLenForEstChannel*(numTags+numAddTags)+1:end-numSymForTailPad);
        
        demodPayloadBits = survey_Mecha_funcDemd(estH,payload_ofdmDerived,payload_ofdmDemod,numTags+numAddTags);
        
        % calculate the number of bits
        for comm_idx1 = 1:(numTags+numAddTags)
            if comm_idx1>=1 && comm_idx1<=numTags
                tmp_addTags = randi([0,1],seqLenForEstChannel*numAddTags,1);
                dePayloadData = [tmp_addTags;demodPayloadBits(:,comm_idx1)];
                numBitErrs(i,comm_idx1) = numBitErrs(i,comm_idx1) + biterr(actualPayloadBits(:,comm_idx1),dePayloadData);
                dePayloadData = [];
            else
                numBitErrs(i,comm_idx1) = numBitErrs(i,comm_idx1) + biterr(actualPayloadBits_addTags(:,comm_idx1-numTags),demodPayloadBits(:,comm_idx1));
            end       
        end
        n = n+1;
    ...
    for comm_idx2 = 1:(numTags+numAddTags)
