...
for i = 1:S
    ...
    while  n<=maxNumPackets
        disp(['Delay: ',num2str(delay(i)),' us -> ','n: ',num2str(n),'-th packet']);
        %%%%%*** TX side ***%%%%%
        ...
        %%%%%***** Backscatter channel & AWGN & delay ***%%%%%
        ...
        tmp_zeros = zeros(delayPoints,1);
        bxSig{2} = [tmp_zeros;bxSig{2}];

        %%%%%*** RX side ***%%%%%
        rx = complex(zeros(length(bxSig{2}),1));
        for rx_idx1 = 1:numTags
            [ttmp_l,ttmp_r] = size(bxSig{rx_idx1}');
            rx(ttmp_l:ttmp_r) = rx(ttmp_l:ttmp_r) + bxSig{rx_idx1};
        end
